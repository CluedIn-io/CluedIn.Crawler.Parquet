...
namespace CluedIn.Provider.Parquet
{
    public class ParquetProvider : ProviderBase, IExtendedProviderMetadata
    {
        private readonly IParquetClientFactory _parquetClientFactory;

        public ParquetProvider([NotNull] ApplicationContext appContext, IParquetClientFactory parquetClientFactory)
            : base(appContext, ParquetConstants.CreateProviderMetadata())
        {
            _parquetClientFactory = parquetClientFactory;
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            var parquetCrawlJobData = new ParquetCrawlJobData();
            if (configuration.ContainsKey(ParquetConstants.KeyName.ApiKey))
            { parquetCrawlJobData.ApiKey = configuration[ParquetConstants.KeyName.ApiKey].ToString(); }

            return await Task.FromResult(parquetCrawlJobData);
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            if (jobData is ParquetCrawlJobData parquetCrawlJobData)
            {
                ...
                dictionary.Add(ParquetConstants.KeyName.ApiKey, parquetCrawlJobData.ApiKey);
            }
        ...
        public string Icon => ParquetConstants.IconResourceName;
        public string Domain { get; } = ParquetConstants.Uri;
        public string About { get; } = ParquetConstants.CrawlerDescription;
        public AuthMethods AuthMethods { get; } = ParquetConstants.AuthMethods;
        public IEnumerable<Control> Properties => null;
        public string ServiceType { get; } = JsonConvert.SerializeObject(ParquetConstants.ServiceType);
        public string Aliases { get; } = JsonConvert.SerializeObject(ParquetConstants.Aliases);
        public Guide Guide { get; set; } = new Guide
        {
            Instructions = ParquetConstants.Instructions,
            Value = new List<string> { ParquetConstants.CrawlerDescription },
            Details = ParquetConstants.Details

        };

        public string Details { get; set; } = ParquetConstants.Details;
        public string Category { get; set; } = ParquetConstants.Category;
        public new IntegrationType Type { get; set; } = ParquetConstants.Type;
    }
}
