...
{
    public class GetCrawlJobDataBehaviour : ParquetProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(ParquetConstants.KeyName.ApiKey, nameof(ParquetCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task ParquetCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Parquet.Core.ParquetCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
